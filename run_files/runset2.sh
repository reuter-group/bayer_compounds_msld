...
### timeout -s SIGINT 8h
echo "run$i started"
#mpirun -np $(($nnodes * $nreps)) -x OMP_NUM_THREADS=$nnodes --bind-to none --bynode 
mpirun -np $nreps -x OMP_NUM_THREADS=$nnodes --map-by node $CHARMMEXEC iflat=$i esteps=25000 nsteps=50000 seed=$RANDOM -i ../msld_flat.inp > output 2> error
...
# run it in conda environment py2 
